...
class ASP_OC_Module(nn.Module):
    def __init__(
        ...
        self.context = nn.Sequential(
            nn.Conv2d(
                features, out_features, kernel_size=3, padding=1, dilation=1, bias=True
            ),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(
                features, out_features, kernel_size=1, padding=0, dilation=1, bias=False
            ),
            ModuleHelper.BNReLU(out_features, bn_type=bn_type),
        )
...
if __name__ == "__main__":
    ...
    aspoc_infer = ASP_OC_Module(512, 256, bn_type="torchsyncbn")
    ...
    for i in range(100):
        ...
        avg_mem += (
            torch.cuda.memory_allocated() - feats.element_size() * feats.nelement()
        )

    print(
        "Average Parameters : {}".format(
            count_parameters(aspoc_infer) + count_parameters(conv_3x3)
        )
    )
