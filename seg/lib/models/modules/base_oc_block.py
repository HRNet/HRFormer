...
class SelfAttentionBlock2D(_SelfAttentionBlock):
    def __init__(
        ...
        super(SelfAttentionBlock2D, self).__init__(
            in_channels, key_channels, value_channels, out_channels, scale, bn_type
        )
...
class BaseOC_Module(nn.Module):
    ...
    def _make_stage(
        self, in_channels, output_channels, key_channels, value_channels, size, bn_type
    ):
...
if __name__ == "__main__":
    ...
    print(
        "Average Parameters : {}".format(
            count_parameters(baseoc_infer) + count_parameters(conv_3x3)
        )
    )
