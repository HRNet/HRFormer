...
if __name__ == "__main__":
    ...
    # ***********  Params for test flops.  **********
    parser.add_argument(
        "--fig_num", default=5, type=int, help="The number of evaluated images."
    )
    parser.add_argument(
        "--shape",
        default=[1024, 512],
    ...
    if configer.get("method") == "fcn_segmentor":
        if configer.get("phase") == "train":
            ...
            model = Tester(configer)
        elif configer.get("phase") == "test_flops":
            from segmentor.tester_flops import TesterFLOPSFixedSize

            model = TesterFLOPSFixedSize(configer, shape=args_parser.shape)
    else:
    ...
    if configer.get("phase") == "train":
        ...
        configer.get("phase").startswith("test")
        and configer.get("network", "resume") is not None
    ):
        model.test()
    elif configer.get("phase") == "test_flops":
        model.test()
